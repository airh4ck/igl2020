...
-/
def predicate_is_definable {L₁ L₂ : lang} {n : ℕ+} (M₁ : struc L₁) 
                           {M₂ : struc L₂} (univ_eq : M₁.univ = M₂.univ) 
                           {p : L₂.R n} (p_int : M₂.R p) :=
  ∃ φ : formula L₁, 
    φ.count_free_vars = n ∧ 
    (∀ v : vector M₂.univ n, 
...
#eval definable_predicates N_arith_semiring


open arithmetic_structure
theorem subset_if_predicates_definable {L₁ : arith_lang} (S₁ S₂ : arith_struc L₁) :
  (∀ (p : Σ (n : ℕ+), L₁.to_lang.R n) (p_int : (arithmetic_structure.arith_struc.to_struc S₁).R p.2 ), 
...
begin
  ...
  simp only [definable_predicates],
